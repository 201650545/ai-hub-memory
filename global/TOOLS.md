...
- 坑（[2026-08-14 / 1.8.6]）：
  ...
  - Windows 长文本 → **优先 `browser fill "textarea" "<text>"`**（CDP 设值，React 能识别，发送按钮会自动出现）；base64 + eval 注入（native setter + Event('input')）**对 React 无效**——textarea.value 有值但 React 内部状态不更新，发送按钮不出现。仅在 fill 不可用时退回 eval；
  ...
  - **切换 Extended·新 UI（2026-09-01 vip-14 实测，与上条并存）**：pill 变为 `button.__composer-pill`（文本 Auto）。**opencli 原生 click（ref/CSS/语义三种）与键盘 ArrowDown 都开不了 Radix 菜单**（aria-expanded 恒 false）；必须 eval 合成**完整指针序列**（pointerover→pointermove→pointerdown→mousedown→pointerup→mouseup→click，带 pointerId/pointerType）打在 pill 中心。菜单项：`[role=menuitemradio]` 的 "Auto" / "Thinking• Extended"（徽标=当前档位，已设过 Extended 则直接点它）/ "GPT-5.6 Luna"；"Configure..."（role=menuitem）里改思考程度（用户 9/1 指点：Standard→Extended 走这里）。选完 pill 显示 "Extended"。
...
- **镜像站故障处理（2026-08-28 停用 Kimi 自动兜底 D-GLOBAL-20260828-01；2026-08-29 明确 Claude 亦非默认兜底 D-GLOBAL-20260829-02）**：问诊/评审**主力固定为 GPT 镜像站（Thinking·Extended）**。镜像站不可用/空回复/回复退化时：① **不自动换模型**，Kimi、Claude 一律不自动转；② 向用户报告故障现象、已尝试的账号与会话、建议动作（换账号/换会话/稍后重试同实例——退化通常是**实例侧**而非流程侧），等用户指示；③ 转 Claude 需用户**当次明确同意**（Claude 额度稀缺，不作常规评审渠道）；Kimi 仅用户主动要求时使用。历史：2026-08-15~27 曾用 Kimi K3 自动兜底（D-GLOBAL-20260815-04，已废止）；本条此前误留了废止前的 Kimi 操作步骤文本，与 2026-08-28 修订自相矛盾，2026-08-29 清理。
