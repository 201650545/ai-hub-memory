...
- [D-GLOBAL-20260830-01] **镜像站聊天历史 ≤30 删一半**（用户 2026-08-30 拍板，强制）：ChatGPT 镜像版聊天历史最多 30 条，每到 30 删除一半（保留最近 15 条），取代此前「每 30 删 10」。同步修订：global/TOOLS.md §4 会话纪律。
- [D-GLOBAL-20260830-02] **GPT 问诊提问方法**（用户 2026-08-30 拍板，强制）：① 上传上下文：需告知 GPT 的内容先上传 GitHub 仓库，要求 GPT 强制读取仓库文件（基于系统真实代码/数据给针对性建议）；② 优化提示词：上传内容含核心代码/数据/部分提示词，聊天窗口只留一句话引用，减少提示词量、响应更快；③ 实时监控：GPT 回答中每 15s 轮询一次生成状态；④ 避免打断：等上一轮彻底完成（stop 按钮消失）再发下一轮。
