...
## ⚠️ 读记忆响应协议（强制，Agent 必读必做）

> 当用户让你「读取记忆项目 / 读一下记忆 / 调用 CLI 读取记忆」时，你**必须**按下面的格式先回答，再开始任何项目工作。这是强制要求，不是建议。

**第一步——先输出你对本记忆项目的理解**（用自己的话，3-5 句，覆盖以下要点）：
- 这是什么：多 Agent 共享记忆系统，GitHub 仓库是唯一记忆真源，记忆按项目隔离。
- 怎么组织：projects/<项目ID>/ 下是 STATE(当前状态)/DECISIONS(决策)/CHANGELOG(流水)；global/ 是全局规则(RULES=宪法 / PROJECTS=项目地图 / DECISIONS=全局决策 / TOOLS=工具)。
- 怎么用：读写一律走 `python scripts/memory.py`（bootstrap 入口 / read / write / checkpoint / capture），禁止直接改文件。
- 约定：R18 自动记忆检查（事件触发 + 10 回合兜底）、凭证绝不入记忆、R17 项目决策优先。

**第二步——主动询问用户要开始哪个项目**（列出当前已注册项目 + 新项目选项）：
> 已注册项目：teaching(教学) / courseware(课件) / memory-system(记忆系统) / english-teaching(英语教学) / english-learning(英语学习) / devel-tools(工具链/DSH 开发)。
> 请告诉我：要开始/继续**哪个项目**？如果是全新项目，请说明项目名称，我会先注册。

**第三步——在用户明确项目之前，不要开始任何具体项目工作**（可以读 global 全局层，但不得擅自读写某个项目线）。
