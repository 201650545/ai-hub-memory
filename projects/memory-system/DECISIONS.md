...
- [D-MEMSYS-20260903-02] **Obsidian 五库压两库**：合并为 Vault A=Work（Handbook+项目索引+项目文档）与 Vault B=Memory（单独保留，安全/生命周期边界，1 个不推荐）。合并 vault≠合并 git repo（各项目仓独立 .git）；Work 根不做大 git 仓。（**2026-09-03 已执行**：D:\Work 合并库已建，四项移入各自保留 .git，注册表改 Work+记忆 两库，重启识别成功；见 CHANGELOG。）
