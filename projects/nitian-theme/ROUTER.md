# ROUTER.md — nitian-theme 冷记忆路由索引
...
> STATE 现有 25 条（08-15~08-27），warm 19 条已具备归档潜质，但尚无 archive 正文批次；待批次迁移时同步补齐本表索引行（按维护规则 1，迁移必须同步索引，否则等于永久丢失）。

## Agent 怎么用（两种唤醒）

**A. 临时召回（＝MoE 的一次 expert activation）**
1. 用关键词在本表 `topic_keys` / `entities` 中查找命中行；
2. 按 `archive_ref` 打开正文（如 `archive/projects/nitian-theme/2026/2026-08.md#S-xxxx`）；
